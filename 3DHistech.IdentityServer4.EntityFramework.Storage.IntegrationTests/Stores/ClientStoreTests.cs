	...
	public class ClientStoreTests : IntegrationTest<ClientStoreTests, ConfigurationDbContext, ConfigurationStoreOptions>
	{
		...
		public async Task FindClientByIdAsync_WhenClientExistsWithCollections_ExpectClientReturnedCollections(DbContextOptions<ConfigurationDbContext> options)
		{
			var testClient = new Client
			{
				...
				ClientSecrets = { new Secret("secret".Sha256()) { Description = "description" } },
				...
				AllowedIdentityTokenSigningAlgorithms = new List<string>() { "id_token" },
				BackChannelLogoutUri = "",
				ClientUri = "",
				Description = "description",
				FrontChannelLogoutUri = "",
		...
		public async Task FindClientByIdAsync_WhenClientsExistWithManyCollections_ExpectClientReturnedInUnderFiveSeconds(DbContextOptions<ConfigurationDbContext> options)
		{
			var testClient = new Client
			{
				...
				AllowedIdentityTokenSigningAlgorithms = new List<string>() { "id_token" },
				BackChannelLogoutUri = "dsa",
				ClientUri = "dsa",
				Description = "description",
				FrontChannelLogoutUri = "da",
				LogoUri = "dsa",
				PairWiseSubjectSalt = "dsa",
				UserCodeType = "das"
			};
			...
			using (var context = new ConfigurationDbContext(options, StoreOptions))
			{
				...
				for (int i = 0; i < 50; i++)
				{
					context.Clients.Add(new Client
					{
						...
						AllowedIdentityTokenSigningAlgorithms = testClient.AllowedIdentityTokenSigningAlgorithms,
						BackChannelLogoutUri = testClient.BackChannelLogoutUri,
						ClientUri = testClient.ClientUri,
						Description = testClient.Description,
						FrontChannelLogoutUri = testClient.FrontChannelLogoutUri,
						LogoUri = testClient.LogoUri,
						PairWiseSubjectSalt = testClient.PairWiseSubjectSalt,
						UserCodeType = testClient.UserCodeType
					}.ToEntity());
				}
